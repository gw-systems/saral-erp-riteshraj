"""
Real-time sync progress tracking for Google Ads
...
class SyncProgressTracker:
    ...
    Usage:
        tracker = SyncProgressTracker(token_id=123, sync_type='historical')
        tracker.start()
        tracker.update(message="Syncing campaigns...", campaigns=50)
    ...
    def __init__(self, token_id, sync_type='incremental'):
        ...
        self.cache_key = f'google_ads_sync_progress_{token_id}'
    ...
    def update(self, message=None, progress_percentage=None, **stats):
        ...
        Args:
            ...
            **stats: Dict of stat counters (campaigns, performance, search_terms, etc.)
        """
        ...
        for key, value in stats.items():
            if isinstance(value, dict):
                # Nested stat (e.g., campaigns={'created': 10, 'updated': 5})
...
def get_sync_progress(token_id):
    ...
    cache_key = f'google_ads_sync_progress_{token_id}'
    ...
    # Always fetch most recent batch log (running OR completed/failed) so logs
    # remain visible after the sync finishes, not just while it's running.
    try:
        batch_log = SyncLog.objects.filter(
            integration='google_ads',
            sync_type__in=['google_ads', 'google_ads_historical', 'google_ads_full'],
            log_kind='batch',
        ).order_by('-started_at').first()
    except Exception as e:
        logger.error(f"Failed to fetch Google Ads batch log: {e}")
        batch_log = None
    ...
    if status == 'completed' and not progress.get('current_status'):
        ...
        progress['current_status'] = f'✅ Sync complete — {records} records synced'
