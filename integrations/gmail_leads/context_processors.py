"""
Context processors for Gmail Leads
...
def active_syncs(request):
    """
    Add active Gmail Leads syncs to template context
    ...
    cache_key = 'gmail_leads_active_tokens'
    tokens_data = cache.get(cache_key)

    if tokens_data is None:
        # Cache miss — query DB
        tokens = GmailLeadsToken.objects.filter(is_active=True).values('id', 'email_account')
    ...
    result_cache_key = 'gmail_leads_active_syncs_ctx'
    ...
    for token_data in tokens_data:
        ...
        if progress and progress.get('status') == 'running':
            active_syncs_list.append({
                ...
                'email_account': token_data['email_account'],
                'progress': progress
            })

    result = {
        'gmail_leads_active_syncs': active_syncs_list,
        'has_active_gmail_sync': len(active_syncs_list) > 0
    }
