"""
Manual full-sync checker for all supported apps.
...
def test_google_ads_full_sync():
    ...
    try:
        ...
        if not tokens.exists():
            print(f"{Colors.WARNING}âš  No active Google Ads tokens - SKIPPED{Colors.ENDC}")
            return None
        ...
        print(f"\n{Colors.OKGREEN}âœ“ Google Ads sync completed in {format_duration(duration)}{Colors.ENDC}")
        return duration

    except Exception as e:
        print(f"{Colors.FAIL}âœ— Google Ads sync failed: {e}{Colors.ENDC}")
        return None
...
def test_gmail_leads_full_sync():
    ...
    try:
        ...
        print(f"\n{Colors.OKGREEN}âœ“ Gmail Leads sync completed in {format_duration(duration)}{Colors.ENDC}")
        return duration

    except Exception as e:
        print(f"{Colors.FAIL}âœ— Gmail Leads sync failed: {e}{Colors.ENDC}")
        import traceback
        traceback.print_exc()
        return None
...
def test_gmail_full_sync():
    ...
    try:
        ...
        print(f"\n{Colors.OKGREEN}âœ“ Gmail sync completed in {format_duration(duration)}{Colors.ENDC}")
        return duration

    except Exception as e:
        print(f"{Colors.FAIL}âœ— Gmail sync failed: {e}{Colors.ENDC}")
        return None
...
def test_bigin_full_sync():
    ...
    try:
        ...
        print(f"\n{Colors.OKGREEN}âœ“ Bigin sync completed in {format_duration(duration)}{Colors.ENDC}")
        return duration

    except RuntimeError as e:
        if "already running" in str(e):
            print(f"{Colors.WARNING}âš  Another sync already running - SKIPPED{Colors.ENDC}")
        else:
            print(f"{Colors.FAIL}âœ— Bigin sync failed: {e}{Colors.ENDC}")
        return None
    except Exception as e:
        print(f"{Colors.FAIL}âœ— Bigin sync failed: {e}{Colors.ENDC}")
        import traceback
...
def test_tallysync_full_sync():
    ...
    try:
        ...
        if not companies.exists():
            print(f"{Colors.WARNING}âš  No active Tally companies - SKIPPED{Colors.ENDC}")
            return None
        ...
        duration = time.time() - start_time
        print(f"\n{Colors.OKGREEN}âœ“ TallySync completed in {format_duration(duration)}{Colors.ENDC}")
        return duration

    except Exception as e:
        print(f"{Colors.FAIL}âœ— TallySync failed: {e}{Colors.ENDC}")
        import traceback
        traceback.print_exc()
        return None
...
def test_callyzer_full_sync():
    ...
    try:
        ...
        print(f"\n{Colors.OKGREEN}âœ“ Callyzer sync completed in {format_duration(duration)}{Colors.ENDC}")
        return duration

    except Exception as e:
        print(f"{Colors.FAIL}âœ— Callyzer sync failed: {e}{Colors.ENDC}")
        return None
...
def main():
    ...
    for app, duration in results.items():
        if duration is not None:
            ...
            status = f"{Colors.WARNING}âš  Skipped/Failed{Colors.ENDC}"
            time_str = "N/A"
